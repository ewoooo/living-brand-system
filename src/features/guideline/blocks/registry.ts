...
import { SectionBlock } from './section/schema'
import {
	type BlockLayout,
	baseContentFields,
	presetFields,
 ...
 * 설명, 그리고 사전 정의 값(`presets`)뿐이다. section은 슈거가 아니라 base + 앵커다.
 *
...
export function blockSchema(entry: BlockEntry): Block {
	// 🔴 임시 예외: section은 아직 leaf `children`을 갖는 옛 스키마다. 기존 문서의 leaf를 카드로 이관한 뒤
	//    앵커 필드 + 카드 필드로 바꾸면 이 분기는 사라진다(expand → backfill → contract).
	if (entry.id === 'section') return SectionBlock
	...
	return {
		...
		fields: presetFields(baseContentFields(), presets),
	}
}
