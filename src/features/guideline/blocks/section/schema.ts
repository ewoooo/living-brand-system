...
import { GUIDELINE_LEAVES } from '../../leaves/registry'
...
// 토픽 한 화면 안의 섹션. 2026-08-26까지는 별도 문서(3단계 '페이지')였고, 그 문서가 이 블록이 됐다.
//
// 🔴 섹션 경계를 **데이터로** 갖는 것이 이 블록의 존재 이유다. 평평한 leaf 목록에서 "제목이 있으면
//    새 섹션"처럼 관례로 표현하면, Better Editor의 드래그 한 번에 경계가 조용히 깨진다.
// 🔴 섹션은 leaf(이미지·위젯)를 **직접** 품는다(2026-09-04). 그 사이에 있던 블록 층은 "몇 열로 놓을지"와
//    "뒤에 색 판을 깔지"만 정하고 있었고, 섹션 26개 중 14개에서는 통과 계층이었다. 폭은 leaf의 `span`이
//    말하고 판(배경)은 두지 않는다 — 브랜드 면은 위젯이 자기 규정으로 그린다.
// 🔴 제목은 선택이다. 제목 없는 섹션은 h2·앵커·목차 항목을 만들지 않는다 — 첫 섹션 위에 히어로 락업만
//    놓는 자리가 이것이다(자회사·해외지사 CI).
...
export const SectionBlock: Block = {
	...
	fields: [
		...
		},
		{
			...
			admin: {
				description:
					'목차에 표시되는 섹션 제목입니다. 비우면 제목·앵커·목차 없이 leaf만 놓입니다(히어로용).',
			},
		},
		{
			name: 'description',
		...
		},
		{
			...
			type: 'blocks',
			label: 'Leaf',
			blocks: GUIDELINE_LEAVES,
			admin: {
				description:
					'이 섹션이 품는 이미지·위젯입니다. 각 leaf의 폭(전폭·절반·삼분)에 따라 줄이 바뀝니다.',
			},
		},
