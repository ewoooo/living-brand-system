...
//
// 🔴 섹션 경계를 **데이터로** 갖는 것이 이 블록의 존재 이유다. 평평한 블록 목록에서 "제목이 있으면
//    새 섹션"처럼 관례로 표현하면, Better Editor의 블록 드래그 한 번에 경계가 조용히 깨진다.
...
export const SectionBlock: Block = {
	...
	fields: [
		...
		},
		{
			...
			required: true,
			localized: true,
			admin: { description: '목차에 표시되는 섹션 제목입니다.' },
		},
		{
			name: 'description',
			type: 'richText',
		...
		//    자식 블록의 면은 배치 영역에서 끊기므로 이 자리를 대신할 수 없다.
		{
			...
			admin: { description: '섹션 전체(제목·본문·블록)를 덮는 배경색입니다. 비우면 기본.' },
		},
		backgroundToneField(),
		{
			name: 'blocks',
