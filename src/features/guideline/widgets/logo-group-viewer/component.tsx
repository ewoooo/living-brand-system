...
//
// 위젯(서버): brand-logos에서 로고들을 뽑아 로고 그룹으로 조립해 LogoGroupView(클라 인터랙션)에 순수 props 주입.
// author 인스턴스(logos/topics/title) 없이 자족 렌더 — 프레임/헤더/설명 산문은 컨테이너 Block이 소유하므로 위젯은 시각/인터랙션만.
// 임계값·토픽은 블록 기본값을 하드코딩. 오버레이(®·클리어스페이스)는 author 에셋이라 여기선 null로 우아하게 강등.
...
export async function LogoGroupViewerWidget() {
	...
	const logos: LogoGroupItem[] = docs
		.map((doc) => ({
			id: String(doc.id),
			label: doc.name ?? null,
			logo: doc.url ?? (doc.filename ? `/api/brand-logos/file/${doc.filename}` : ''),
			registeredMark: null,
