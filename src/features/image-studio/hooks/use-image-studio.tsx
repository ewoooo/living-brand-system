...
export function ImageStudioProvider({
	...
	const colorValue: ImageColorAdjustment | null =
		typeof lineColor === 'string'
			...
			: null
	const canDownload = colorValue ? config.output.formats.includes('png') : config.output.original
		...
		execute: async (action) => {
			if (action === 'selected') {
				...
				if (src && selected !== null) {
					await downloadImage(src, selected, config.output, colorValue)
				}
				return
			}
			...
			for (const [index, src] of (result?.images ?? []).entries()) {
				await downloadImage(src, index, config.output, colorValue)
			}
		},
	})
