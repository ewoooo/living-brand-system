...
import { ControllerPicker } from '@/components/studio/shared/controller'
...
/**
 * 자산 피커 본문의 이미지 프로파일 카드 그리드 — 킷(Controller.Picker)이 크롬을, 이 컴포넌트가 도메인을 갖는다.
 * 컨텍스트의 교체 후보와 현재 계약만 읽고, 카드를 고르면 프로파일을 교체한다.
 * 고른 뒤 닫기는 카드를 감싼 Controller.Picker.Close가 받는다 — 열림 상태는 킷이 소유한다.
	...
	return (
		<div data-slot="image-profile-picker" className="grid shrink-0 grid-cols-3 gap-3 pr-1">
			{profiles.options.map((option) => {
				...
				return (
					<ControllerPicker.Close key={option.profileId} asChild>
						<button
							type="button"
