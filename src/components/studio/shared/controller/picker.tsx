...
const PickerFrameContext = React.createContext<HTMLElement | null>(null)

type ControllerPickerRootProps = {
	className?: string
	children: React.ReactNode
}

/**
 * 피커 컴파운드의 뿌리 — 열림 상태와 패널의 위치 기준을 함께 소유한다.
 *
 ...
 */
function ControllerPickerRoot({ className, children }: ControllerPickerRootProps) {
	...
	return (
		<DialogPrimitive.Root modal={false}>
			<div
				ref={setFrame}
				data-slot="controller-picker"
				className={cn('relative h-full', className)}
			>
				<PickerFrameContext.Provider value={frame}>{children}</PickerFrameContext.Provider>
			</div>
		</DialogPrimitive.Root>
	)
}

/** 패널을 여는 버튼 자리 — 피커 안에만 존재한다(무엇을 여는지가 계약으로 남는다). */
function ControllerPickerTrigger({
	...props
}: React.ComponentProps<typeof DialogPrimitive.Trigger>) {
	return <DialogPrimitive.Trigger data-slot="controller-picker-trigger" {...props} />
}

/** 고르면 닫히는 자리 — 패널 본문의 선택 버튼이 asChild로 감싸 쓴다(닫기는 radix가 소유). */
function ControllerPickerClose({ ...props }: React.ComponentProps<typeof DialogPrimitive.Close>) {
	return <DialogPrimitive.Close data-slot="controller-picker-close" {...props} />
}

type ControllerPickerPanelProps = {
 ...
 */
function ControllerPickerPanel({ tabs, empty, className, children }: ControllerPickerPanelProps) {
	const frame = React.useContext(PickerFrameContext)

	const content = (
		<DialogPrimitive.Content
			data-slot="controller-picker-panel"
...
/**
 * 탭을 가진 플로팅 자산 피커 — 트리거는 이 컴파운드 안에만 존재한다.
 * 카드 + 열기 버튼 형태의 소비자는 Controller.Browser가 이 파츠들을 조립해 만든다.
 */
export const ControllerPicker = {
	Root: ControllerPickerRoot,
	Trigger: ControllerPickerTrigger,
	Panel: ControllerPickerPanel,
	Close: ControllerPickerClose,
}

export {
	ControllerPickerClose,
	ControllerPickerPanel,
	ControllerPickerRoot,
	ControllerPickerTrigger,
}
