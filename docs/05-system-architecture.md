...
이 갈림은 **선언이 아니라 Next의 추론 결과입니다.** 챕터 페이지가 동적인 이유는 미리보기(`draftMode`)와 권한 확인(`headers`)을 읽기 때문이고, 신선함을 위해 고른 것이 아닙니다. 따라서 어느 라우트에서 그 호출을 넣거나 빼면 성질이 조용히 뒤집힙니다. 정적 여부는 빌드 출력의 `○`/`ƒ` 표시로 확인합니다.
