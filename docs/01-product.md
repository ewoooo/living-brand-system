...
이 제품의 이름은 Living Design System(이하 LDS)입니다.
LDS의 목표는 작업 과정에서 필요한 기준을 적시에 안내하고, 사용 기록과 검수 기록을 운영자가 확인할 수 있는 디지털 가이드라인을 만드는 것입니다.

정책과 규칙은 문서에서 끝나지 않고, 실제 제작 과정에서 Agent를 통해 활용되며, 에셋 제너레이션 기록과 검수 기록으로 관리됩니다.
...
- 어떤 규칙이 위반되었는지 제작 맥락과 함께 확인하기 어렵습니다.
...
- 현장의 에셋 제너레이션 기록을 운영자가 확인하기 어렵습니다.

가이드라인은 일방향 전달에 머물고, 실제 사용 과정과 함께 운영되기 어려운 상황입니다.

## 3. 서비스 흐름

서비스 목표는 가이드라인이 실제 제작에 자주 쓰이고, 에셋 제너레이션 기록과 검수 기록이 운영자가 확인할 수 있는 상태로 남는 구조를 만드는 것입니다.

### 참여자 흐름

서비스 흐름은 네 참여자의 흐름으로 봅니다.
Manager, System, Agent, Creator가 각각 하나의 축이 되고, 시간은 위에서 아래로 흐릅니다.

S1과 S2는 같은 System의 서로 다른 역할입니다.
S1은 발행된 기준을 사용자에게 제공하기 위한 역할이고, S2는 사용자가 남긴 에셋 제너레이션과 검수 기록을 보관하는 역할입니다.
Agent는 제작 상황에 맞는 안내와 점검을 제공하는 제품 기능입니다.
...
sequenceDiagram
  ...
  M->>S: 정책·규칙·에셋·템플릿 등록
  Note over S: S1. 기준 구조화·Official Version 관리
  S->>A: 구조화된 기준·Official Version 제공
  A->>C: 제작 맥락에 맞춘 안내·점검
  C->>S: 에셋 제너레이션 결과·사용 행동 기록
  Note over S: S2. 에셋 제너레이션·검수 기록 저장
  M->>S: 에셋 제너레이션·검수 이력 확인
```

### 운영 구조

같은 흐름을 운영 구조로 보면 다음과 같습니다.
이 다이어그램은 발행된 가이드라인이 제작 지원과 검수 기록으로 이어지는 구조를 보여줍니다.

```mermaid
flowchart LR
  M["Manager<br/>브랜드 관리자"]
  S1["System<br/>S1. 기준 구조화·Official Version 관리"]
  A1["Agent<br/>사용자 지원"]
  C["Creator<br/>제작·질문·검수"]
  S2["System<br/>S2. 에셋 제너레이션·검수 기록 저장"]

  M -->|"정책·규칙·에셋·템플릿 등록"| S1
  S1 -->|"구조화된 기준·Official Version 제공"| A1
  A1 -->|"제작 맥락에 맞춘 안내·점검"| C
  C -->|"에셋 제너레이션 결과·사용 행동 기록"| S2
  S2 -->|"에셋 제너레이션·검수 이력 제공"| M
```
...
| System | 기준을 구조화하고, Official Version과 사용 기록을 관리합니다. |
| Agent | 작업 상황에 맞는 가이드와 점검을 제공합니다. |
| Creator | 가이드를 활용해 산출물을 만들고, 질문과 검수 결과를 남깁니다. |

### 흐름별 기록

| 흐름 | 액터 | 남는 기록 | 다음 액터가 하는 일 |
| --- | --- | --- | --- |
| Manager -> System | Manager | 정책, 규칙, 에셋, 템플릿, Official Version | System은 기준을 구조화하고 적용 범위를 연결합니다. |
| System -> Agent | System | 구조화된 기준, 어플리케이션 타입, VersionRef | Agent는 제작 맥락에 맞는 안내와 점검 기준을 만듭니다. |
| Agent -> Creator | Agent | 실행 가이드, 점검 결과, 수정 지시 | Creator는 안내를 보고 제작하고 검수합니다. |
| Creator -> System | Creator | 에셋 제너레이션 결과, 질문, 검수 결과, 화면 행동 기록 | System은 에셋 제너레이션과 검수 이력을 저장합니다. |
| Manager -> System | Manager | 에셋 제너레이션 이력 조회 조건, 검수 이력 조회 조건 | System은 운영자가 확인할 기록을 제공합니다. |
...
| 근거 | Agent가 참고한 정책, 규칙, Official Version |
...
| --- | --- | --- |
| 에셋 제너레이션 결과 | 정책/규칙 위반 사항, 산출물 점검 결과, 검수 실패 사유, 수정 이력, 질문 사항 | 제작 흐름과 검수 이력 |
| 사용 행동 | 조회한 항목, 체류한 구간, 다운로드한 에셋 | 화면 행동과 자원 사용 기록 |

이 정보가 쌓이면 Manager는 제작 흐름과 검수 이력을 운영 조회 화면에서 확인할 수 있습니다.
...
| 검수 전 점검 | 입력값과 산출물이 규칙을 충족하는지 확인합니다. |
| 쉬운 수정 지시 | 사용자가 어디를 어떻게 고쳐야 하는지 알려줍니다. |
| 운영 기록 조회 | 에셋 제너레이션, 답변, 검수, 화면 행동 기록을 운영자가 확인합니다. |
...
- 사용자가 가이드라인을 직접 해석하지 않아도 제작을 수행할 수 있습니다.
- 질문, 점검 결과, 검수 실패 사유를 운영 기록으로 남길 수 있습니다.
- 실제 사용 데이터를 바탕으로 제작 흐름과 검수 상태를 확인할 수 있습니다.
...
| 문서 | 이어서 정의할 내용 |
| --- | --- |
| 유즈케이스 | Manager, System, Agent, Creator가 어떤 입력을 만들고 어떤 결과를 받는지 정의합니다. |
| 데이터 생명주기 | 정책, 규칙, 산출물, 점검 결과, 사용 기록이 어떤 상태로 변하는지 정의합니다. |
| 도메인 모델 | 정책, 규칙, 템플릿, 에셋 제너레이션, 점검, 수정 안내의 개념과 관계를 정의합니다. |
