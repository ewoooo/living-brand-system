...
거버넌스와 규칙은 문서에서 끝나지 않고, 실제 작업 과정에서 Agent를 통해 활용되며, 사용자 피드백과 운영 데이터를 바탕으로 계속 개선됩니다.

이 문서에서 기준은 거버넌스와 규칙을 함께 가리키는 말입니다.
거버넌스는 브랜드가 지켜야 하는 상위 원칙이고, 규칙은 거버넌스를 실제 작업에서 확인할 수 있게 낮춘 판단 단위입니다.
...
- 거버넌스와 규칙의 추가·수정 이유를 추적하기 어렵습니다.
...
| 참여자 | 역할 |
| --- | --- |
| Manager | 거버넌스, 규칙, 에셋, 템플릿을 관리하고 최종 의사결정을 담당합니다. |
...
| --- | --- | --- | --- |
| Manager -> System | Manager | 거버넌스, 규칙, 에셋, 템플릿, 버전 | System은 기준을 구조화하고 적용 범위를 연결합니다. |
...
| System -> Agent | System | 거버넌스/규칙 위반 사항, 반려 사유, 자주 본 항목 | Agent는 반복 문제와 개선 후보를 정리합니다. |
...
| --- | --- | --- |
| 작업 결과 | 거버넌스/규칙 위반 사항, 제출물 점검 결과, 반려 사유, 수정 이력, 질문 사항 | 반복 위반, 반복 질문, 자주 반려되는 기준 |
...
다만 Agent가 거버넌스와 규칙을 직접 변경하지는 않습니다.
...
이 제품은 현장 작업자가 거버넌스와 규칙을 직접 해석하지 않아도 작업을 완료할 수 있게 돕습니다.
...
| 제출 전 점검 | 입력값과 작업물이 규칙을 충족하는지 확인합니다. |
| 쉬운 수정 지시 | 사용자가 어디를 어떻게 고쳐야 하는지 알려줍니다. |
| Manager 피드백 연결 | 검토 코멘트를 규칙과 위반 항목에 연결합니다. |
...
- 반려 사유를 거버넌스와 규칙에 연결해 관리할 수 있습니다.
- 반복적으로 발생하는 문제를 바탕으로 거버넌스, 규칙, 템플릿을 개선할 수 있습니다.
...
- 실제 사용 데이터를 바탕으로 거버넌스와 규칙을 지속적으로 개선할 수 있습니다.
...
| 데이터 생명주기 | 거버넌스, 규칙, 작업물, 점검 결과, 사용 기록, 개선 후보가 어떤 상태로 변하는지 정의합니다. |
| 도메인 모델 | 거버넌스, 규칙, 템플릿, 작업, 점검, 피드백, 개선 후보의 개념과 관계를 정의합니다. |
