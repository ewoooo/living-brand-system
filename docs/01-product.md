...
| System -> Agent | System | 정책/규칙 위반 사항, 검수 실패 사유, 자주 본 항목 | Agent는 반복 문제와 개선 후보를 정리합니다. |
| Agent -> Manager | Agent | 반복 질문, 반복 위반, 자원 사용 문제, 개선 후보 | Manager는 채택 여부를 결정하고 기준을 개정합니다. |
...
| --- | --- | --- |
| 작업 결과 | 정책/규칙 위반 사항, 산출물 점검 결과, 검수 실패 사유, 수정 이력, 질문 사항 | 반복 위반, 반복 질문, 자주 실패하는 기준 |
...
Agent는 사용 기록을 분석해 반복 질문, 반복 위반, 예외 사례, 자원 사용 문제를 개선 후보로 정리합니다.
...
| 피드백 적용의 어려움 | 수정 권장을 보고도 무엇을 수정해야 하는지 모를 수 있습니다. |
...
검수 실패 사유와 수정 방법을 함께 제공하면 동일한 오류의 재발이 감소할 것입니다.
...
| 인사이트 보고서 | 반복 질문, 반복 위반, 반복 검수 실패 사유를 모아 개선 후보로 만듭니다. |
...
- 질문, 점검 결과, 검수 실패 사유를 가이드라인 개선에 활용할 수 있습니다.
