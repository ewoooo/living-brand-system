...
정책과 규칙은 문서에서 끝나지 않고, 실제 작업 과정에서 Agent를 통해 활용되며, 사용자 피드백과 운영 데이터를 바탕으로 계속 개선됩니다.
...
| --- | --- | --- |
| 작업 결과 | 정책/규칙 위반 사항, 제출물 점검 결과, 반려 사유, 수정 이력, 질문 사항 | 반복 위반, 반복 질문, 자주 반려되는 기준 |
...
다만 Agent가 정책과 규칙을 직접 변경하지는 않습니다.
...
- 반려 사유를 정책과 규칙에 연결해 관리할 수 있습니다.
- 반복적으로 발생하는 문제를 바탕으로 정책, 규칙, 템플릿을 개선할 수 있습니다.
...
- 실제 사용 데이터를 바탕으로 정책과 규칙을 지속적으로 개선할 수 있습니다.
