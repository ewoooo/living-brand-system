...
- 실제 triage API 경계는 `scripts/smoke-agent-chat.ts`로 확인합니다. `AGENT_CHAT_TRIAGE_ENABLED=true`인 실행 중 앱과 동일한 DB를 `PAYLOAD_DB_PUSH=false`로 연결하고 인증 쿠키를 `AGENT_CHAT_SMOKE_COOKIE`에 전달합니다. 특정 케이스만 확인하려면 `AGENT_CHAT_SMOKE_CASE`에 `quick`, `lookup`, `research`, `action`, `high-risk-action` 중 하나를 지정합니다.
