...
**카드가 소유하는 것** — 규격 비율(`ratio`), 디스플레이 하나(정적 이미지 또는 위젯), 선택 판정 표식(`mark`: none·do·ok·dont), 캡션(제목·설명, 각각 선택)과 캡션 배치(카드 아래·판 위 하단). 배치·텍스트 스타일은 [09 §6](09-design-system.md)가 소유합니다. 위젯은 카드 디스플레이의 한 종류이고 판 안을 채우기만 합니다(`cards/`).
