...
**카드가 소유하는 것** — 규격 비율(`ratio`), 디스플레이 하나(정적 이미지 또는 위젯), 캡션(제목·설명, 각각 선택). 위젯은 카드 디스플레이의 한 종류이고 판 안을 채우기만 합니다(`cards/`).
...
- ✅ **섹션의 자식 위젯 이미지는 AI 검수에 넣지 않습니다 — 결함이 아니라 결정입니다**(2026-08-12). 기계(AI 챗·검색·검수)가 읽는 것은 섹션이 소유한 title·description·rule 셋뿐이고, 자식 위젯과 그 이미지는 사람이 보는 표현입니다(§4). 그래서 `blocks/shared/card-projection.ts`는 `referenceAssets: []`를 돌려주고, `checks/collect-guideline-check-sources.ts`도 leaf 이미지를 모으지 않습니다. 🔴 **위젯별 projection을 만들어 이 경로를 "복구"하지 마십시오.**
