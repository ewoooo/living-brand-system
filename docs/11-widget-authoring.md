...
🔴 **모듈 스코프 스토어는 금지입니다.** 토픽 라우트가 여러 섹션를 한 화면에 렌더하므로, 섹션마다 놓인 패널이 전부 같은 값을 물어 슬라이더 하나가 판형 12개를 함께 움직입니다(실측된 사고). `set`은 `useCallback`으로 안정화해 소비자가 effect 의존에 넣을 수 있게 합니다.
