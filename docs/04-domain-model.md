...
기준은 별도 모델이라기보다 정책과 룰을 함께 부르는 상위 표현입니다.
System과 Agent가 실제로 참조해야 하는 단위는 가능한 한 정책과 룰로 나누어 저장합니다.
...
룰은 항상 수치화될 필요는 없습니다. 다만 정책보다 실제적이어야 합니다.
...
에셋은 룰이 아닙니다. 에셋은 사용하거나 참조하는 대상입니다.

룰은 에셋, 템플릿, 입력값, 작업물의 사용 가능 여부를 판단하는 근거입니다.
...
공식 에셋은 Governance 쪽 데이터입니다. 업로드 자산은 Work 쪽 데이터입니다.
...
| 조회 항목 | Consumer가 열람한 정책, 룰, 예시, FAQ, 템플릿 |
...
| System Rules | 시각/언어 시스템 룰 | 정책을 디자인 요소로 낮춥니다. |
| Applied Context | 실제 산출물 적용 기준 | 정책과 룰을 작업 상황에 적용합니다. |
...
룰이 충돌하거나 해석이 필요한 경우 다음 순서로 판단합니다.
