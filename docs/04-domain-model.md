...
| Concept | Meaning |
| --- | --- |
| 기준 Standard | 거버넌스와 규칙을 함께 가리키는 말 |
| 거버넌스 Governance | 브랜드가 지향하거나 지켜야 하는 상위 기준 |
| 규칙 Rule | 거버넌스를 실제 작업에 적용할 수 있게 낮춘 판단 단위 |
| 점검 Check | 규칙을 작업물이나 입력값에 적용해 확인하는 행위 |
| 피드백 Feedback | 점검 결과를 사용자가 고칠 수 있게 설명한 것 |

기준은 별도 모델이라기보다 거버넌스와 규칙을 함께 부르는 상위 표현입니다.
System과 Agent가 실제로 참조해야 하는 단위는 가능한 한 거버넌스와 규칙으로 나누어 저장합니다.
...
규칙은 항상 수치화될 필요는 없습니다. 다만 거버넌스보다 실제적이어야 합니다.
...
| --- | --- | --- |
| Governance | 무엇이 공식 거버넌스인가? | 거버넌스, 규칙, 카테고리, 에셋, 템플릿, 버전, 예외 |
...
| Evaluation | 작업물이 거버넌스와 규칙에 맞는지 어떻게 판단하는가? | 점검, 위반 항목, 주의 항목, 수정 지시, 피드백, 승인, 반려 |
| Insight | 반복 문제를 어떻게 개선으로 연결하는가? | 반복 패턴, 인사이트, 개선 후보, 거버넌스/규칙 변경, 개선 효과 |
...
| 카테고리 | 거버넌스와 규칙을 찾고 이해하기 쉽게 나누는 분류 |
| 에셋 | 거버넌스와 규칙을 적용할 때 사용하는 파일 또는 참고 자료 |
| 템플릿 | Consumer가 작업을 시작할 때 사용하는 제한된 형식 |
| 버전 | 특정 시점에 공식으로 적용되는 거버넌스와 규칙의 상태 |
| 예외 | 일반 거버넌스나 규칙을 그대로 적용하기 어려울 때 허용되는 별도 판단 |
...
공식 에셋은 거버넌스 쪽 데이터입니다. 업로드 자산은 Work 쪽 데이터입니다.
...
| 조회 항목 | Consumer가 열람한 거버넌스, 규칙, 예시, FAQ, 템플릿 |
...
| Concept | Meaning |
| --- | --- |
| 질문 | Consumer가 작업 상황에서 거버넌스나 규칙 적용을 묻는 자연어 입력 |
| 답변 | Agent 또는 System이 질문에 대해 제공하는 안내 |
| 근거 | 답변, 점검, 피드백이 참조한 거버넌스, 규칙, 자료 |
...
| 승인 | 제출물이 거버넌스와 규칙 기준상 사용할 수 있다고 판단된 상태 |
| 반려 | 제출물이 거버넌스와 규칙 기준상 사용할 수 없다고 판단된 상태 |
...
| 인사이트 | 반복 패턴을 거버넌스 또는 규칙 개선 후보로 해석한 결과 |
| 개선 후보 | Manager가 채택하거나 제외해야 하는 변경 제안 |
| 거버넌스/규칙 변경 | 채택된 인사이트를 바탕으로 공식 거버넌스, 규칙, 템플릿, 안내, 예시를 수정하는 행위 |
| 개선 효과 | 변경 이후 질문 수, 반려율, 수정 요청 수, 반복 오류가 줄었는지 확인한 결과 |

인사이트는 자동으로 거버넌스나 규칙을 바꾸지 않습니다.
...
| Group | Meaning | Role |
| --- | --- | --- |
| Core Governance | 브랜드의 최상위 거버넌스 | 다른 판단의 기준 |
| System Rules | 시각/언어 시스템 규칙 | 거버넌스를 디자인 요소로 낮춥니다. |
| Applied Context | 실제 산출물 적용 기준 | 거버넌스와 규칙을 작업 상황에 적용합니다. |
