...
- 가르는 기준은 **창작자가 바꿀 수 있어야 하는 축인가**입니다. 색, 모양, 속도, 위치처럼 직관적이고 변화폭이 큰 것만 남기고 나머지는 기본값으로 고정합니다. 고정된 축은 manager가 Payload에서 조정합니다.
...
🔴 **컨트롤 선언 자체를 지우지는 않습니다.** 창작자에게 감추더라도 manager는 Payload에서 그 값을 조정할 수 있어야 하고, 선언이 사라지면 그 경로도 함께 사라집니다. 셰이더 변환기가 기본 입력을 깔고 컨트롤 값으로만 덮으므로, 선언을 남긴 채 노출만 좁히면 값은 정본 기본값을 따릅니다.
