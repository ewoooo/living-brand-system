...
의도한 방향은 가이드라인을 직접 읽는 대신 [Review](review.md)를 검증 skill로 쓰고, 이미지가 필요하면 [Image](image.md)를 호출하는 것입니다. 다만 아래 표시대로 현재 구현은 **Figma HTML 템플릿 조합(클라이언트 PNG)**까지이고, 규정 주입·검증·이미지 생성은 로드맵입니다.
...
- 입력: 발행된 템플릿의 `html`/`overrides` + 열린 텍스트 슬롯 `values`. 텍스트 슬롯은 `inputFormat`/`maxLength`/`maxLines`를 강제합니다.
- 출력: 클라이언트 PNG 다운로드. Payload에는 아무것도 쓰지 않음(생성 세션/출력 레코드 없음).

템플릿 상세 페이지는 검증된 HTML이 있는 템플릿만 열고 `HtmlAssetGenerator`로 슬롯을 채운 뒤 PNG를 내려받습니다. JSON 기반 `AssetGenerator`·`TemplateRenderer`·코드 sandbox와 AI Chat JSON 첨부 fallback은 실행 경로에서 제거했습니다. JSON-only 템플릿은 발행할 수 없습니다. `templates.jsonTemplate` 필드는 기존 데이터 호환을 위해 남아 있지만 Create·AI Chat은 읽거나 렌더하지 않습니다.
...
| Surface | 상태 | 진입점 |
| --- | --- | --- |
| [Page](../surfaces/page.md) | 구현 | `/studio/template` → 카테고리 → 템플릿 → HtmlAssetGenerator. 발행된 HTML 템플릿만 읽고 비로그인 공개 읽기 |
...
- 공유 유틸: `exportHtmlToPng`(Create·Chat 공유).
