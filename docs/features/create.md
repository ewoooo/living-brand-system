...
- Payload 컬렉션: `templates`·`template-categories`·`template-assets`, 그리고 `rules`(템플릿의 `templateRules` 인라인 배열 필드로 참조 — 별도 `template-rules` 컬렉션 아님). import은 `brand-logos`·`application-images`를 사용. 보안 게이트: 템플릿의 모든 이미지가 인가된 에셋 컬렉션을 참조해야 저장됨(fail-closed).
