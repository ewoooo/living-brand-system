...
- `HtmlAssetGenerator`(`src/features/asset-generation/components/`): canonical HTML의 열린 텍스트 슬롯을 편집하고 미리보기를 렌더.
- `export-template-png.client`(`src/features/asset-generation/services/`): 검증된 HTML을 Shadow DOM에 구성한 뒤 `html-to-image`로 PNG 캡처 → 브라우저 다운로드.
...
- 출력: 클라이언트 PNG 다운로드. 운영자가 `72`(대형 인쇄)·`150`(일반 용지)·`300`(고급 용지)ppi 중 하나를 지정한 경우 CMYK TIFF도 다운로드할 수 있음. Payload에는 아무것도 쓰지 않음(생성 세션/출력 레코드 없음).

TIFF는 원본 가로·세로 픽셀을 리샘플링하지 않고 PPI 메타데이터만 기록합니다. 따라서 인쇄 크기는 `px ÷ ppi × 25.4mm`로 정해집니다. 투명 영역은 흰색으로 평탄화하고 Sharp 내장 기본 CMYK ICC 프로파일을 삽입합니다. 최대 입력은 `67,108,864`픽셀·PNG 20MB이며, PPI를 설정할 때 픽셀 상한을 Template 저장 hook에서 검증합니다. 벡터 PDF는 후속 범위입니다.
...
- 공유 클라이언트 서비스: `export-template-png.client`(Create·Chat 공유).
