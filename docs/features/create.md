...
- `TemplateCreator`(`src/features/template-create/components/`): canonical HTML의 열린 텍스트 슬롯을 편집하고 미리보기를 렌더.
...
| Surface | 상태 | 진입점 |
| --- | --- | --- |
| [Page](../surfaces/page.md) | 구현 | `/studio/template` → 카테고리 → 템플릿 → TemplateCreator. 발행된 canonical HTML 템플릿만 읽고 비로그인 공개 읽기 |
