...
- 기준 소스: published `guideline-sections`·`guideline-pages`의 문서 및 Block `checks[]`. 실행 시 소유 문서 또는 Block의 전체 정규화 콘텐츠를 evidence로 만들고, referenceAssets·heuristicPrompt·RuleChecker 계약과 함께 `CheckSession.rulesetSnapshot`에 고정합니다.
