...
- **프로파일 생성**: `profileId`가 있으면 published 프로파일의 시스템 프롬프트와 선택적 유저 프롬프트 후보를 읽습니다. 후보가 있으면 Haiku가 각 주제에서 하나를 선택하고, 후보가 없으면 AI 정규화를 생략합니다. 두 경우 모두 유저 인풋 원문을 `subject`로 보존해 flat JSON으로 합칩니다.
...
- **출력 크기**: 프로파일은 공급자와 무관한 프리셋을 소유하고, 생성 서비스가 현재 이미지 공급자의 크기로 변환합니다. 현재 계약은 `square` → `1024x1024`, `landscape` → `1536x1024`, `portrait` → `1024x1536`입니다.
- **최종 프롬프트 JSON**: 유저 인풋 원문을 `subject`로 보존하고 시스템 프롬프트와 유저 프롬프트를 flat JSON으로 합칩니다. 같은 주제는 유저 프롬프트가 우선합니다.
