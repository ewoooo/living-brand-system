...
- **프로파일 생성**: `profileId`가 있으면 published 프로파일의 시스템 프롬프트와 유저 프롬프트 후보를 읽습니다. Haiku가 사용자 요청에 맞춰 각 주제의 후보를 하나씩 선택하고, 유저 인풋 원문을 `subject`로 보존해 flat JSON으로 합칩니다.
...
- **유저 프롬프트**: `주제(key)`, `프롬프트 후보[]`를 정의합니다. AI 구조화 출력은 각 주제마다 후보 중 하나만 선택할 수 있습니다.
