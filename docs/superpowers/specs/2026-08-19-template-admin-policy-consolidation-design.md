...
같은 이유로, `제한` 패널을 템플릿에서 없애면서 `text.color`·`background.color`의 **availability 좁히기**(전체 텍스트 색이나 배경 색을 읽기 전용/비활성으로 만드는 것) 역시 함께 없어진다. 쓰는 템플릿이 0개라 잃는 값은 없지만, 다음 사람이 이것을 버그로 재발견하지 않도록 적어 둔다.
