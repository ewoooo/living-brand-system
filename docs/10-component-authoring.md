...
- **무엇을 그릴지는 편집 계약이 말합니다.** 템플릿이 여는 슬롯·컨트롤 종류·레인지는 kind 판별 유니언의 계약 객체(원형: `template-config.ts`의 `TemplateConfig`, published 원천에서 파생)가 정의하고, 사이드바는 계약만 소비합니다 — 원시 nodeConfigs를 재해석하지 않습니다. 계약에는 정의(defaultValue·레인지)만 싣고 세션 값은 싣지 않습니다.
